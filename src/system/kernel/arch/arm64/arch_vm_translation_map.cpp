	...
	VMPhysicalPageMapper** _physicalPageMapper)
{
	...
	if (hafdbs == ID_AA64MMFR1_HAFDBS_AF_DBS) {
		VMSAv8TranslationMap::fHwFeature = VMSAv8TranslationMap::HW_ACCESS | VMSAv8TranslationMap::HW_DIRTY;
		tcr |= (1UL << 40) | (1UL << 39);
	}
...
arch_vm_translation_map_init_post_area(kernel_args *args)
{
	...
	if (args->arch_args.uart.kind[0] != 0) {
		...
		void* address = (void*)args->arch_args.uart.regs.start;
		area_id area = vm_create_null_area(VMAddressSpace::KernelID(),
...
static void map_page_early(phys_addr_t ptPa, int level, addr_t va, phys_addr_t pa, phys_addr_t (*get_free_page)(kernel_args *), kernel_args *args)
{
...
bool
arch_vm_translation_map_is_kernel_page_accessible(addr_t va,
	uint32 protection)
{
